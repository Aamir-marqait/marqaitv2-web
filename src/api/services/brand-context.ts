...
class BrandContextService {
  ...
  /**
   * Save LLM generated data (brandbook content) to backend
   * This runs in the background without blocking UI
   * @param businessId - The business ID
   * @param llmData - The JSON data generated by frontend LLM analysis
   */
  async saveLLMData(businessId: string, llmData: any): Promise<void> {
    try {
      await axiosInstance.put(
        `${this.baseUrl}/${businessId}/llm-data`,
        { data: llmData }, // Wrap in 'data' field as expected by backend
        { timeout: 0 }
      );
      console.log('✅ LLM generated data saved to backend successfully');
    } catch (error: any) {
      console.error('❌ Failed to save LLM data to backend:', error);
